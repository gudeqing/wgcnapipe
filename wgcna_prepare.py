...
parser.add_argument('-log_base', type=int, metavar="log base", default=2,
                    help="mean expression cutoff")
parser.add_argument('-log_add', type=int, metavar="log base", default=1,)
...
# clean data
exp_pd = pd.read_csv(exp_file, header=0, index_col=0, sep=None, engine='python')
if args.log_base == 2:
    exp_pd = np.log2(exp_pd+args.log_add)
elif args.log_base == 10:
    exp_pd = np.log10(exp_pd + args.log_add)
elif args.log_base == 0 or args.log_base == 1:
    pass
else:
    raise Exception(' log base should be one of  [1, 2, 10] !')
